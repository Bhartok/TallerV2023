...
void BasicTimer_Config(BasicTimer_Handler_t *ptrBTimerHandler){
	...
	if(ptrBTimerHandler->ptrTIMx == TIM2){
		...
		RCC->APB1ENR |= RCC_AP1ENR_TIM2EN;
	}
	else if(ptrBTimerHandler->ptrTIMx == TIM3){
		// Registro del RCC que nos activa la señal de reloj para el TIM3
		RCC->APB1ENR |= RCC_AP1ENR_TIM3EN;
	}
	else{
		__NOP();
	}
	 ...
	 * periodo_incremento * veces_incremento_counter =
 *     prescaler * maximo valor incremento =
	...
	if(ptrBTimerHandler->TIMx_Config.TIMx_mode == BTIMER_MODE_UP){
		...
		/* Escriba codigo aca */
		ptrBTimerHandler->ptrTIMx->CCR1 &= ~TIM_CR1_DIR;
		...
		/* Escriba codigo aca */
		ptrBTimerHandler->CCR1 &= TIM_CR1_DIR;
